...
trait CommonUserFunctions
{
    ...
    {
        if (!empty($value)) {
            $query = User::where('users.user_type', 'candidate');
            $query->where('users.is_active', 1);
            $query->where('users.verified', 1);
            $query->where('users.profile_visibility', 1);
            if ($field == 'industry_id') {
                return $query->where('industry_id', $value)->active()->count('id');
            }
            if ($field == 'job_skill_id') {
                $job_seeker_ids = ProfileSkill::where('job_skill_id', '=', $value)->pluck('user_id')->toArray();
                return $query->whereIn('id', array_unique($job_seeker_ids))->active()->count('id');
            }
            if ($field == 'functional_area_id') {
                return $query->where('functional_area_id', '=', $value)->active()->count('id');
            }
            if ($field == 'careel_level_id') {
                return $query->where('careel_level_id', '=', $value)->active()->count('id');
            }
            if ($field == 'degree_level_id') {
                return $query->where('degree_level_id', '=', $value)->active()->count('id');
            }
            if ($field == 'salary_period_id') {
                return $query->where('salary_period_id', '=', $value)->active()->count('id');
            }
            if ($field == 'gender_id') {
                return $query->where('gender_id', '=', $value)->active()->count('id');
            }
            if ($field == 'job_experience_id') {
                return $query->where('job_experience_id', '=', $value)->active()->count('id');
            }
            if ($field == 'country_id') {
                return $query->where('country_id', '=', $value)->active()->count('id');
            }
            if ($field == 'state_id') {
                return $query->where('state_id', '=', $value)->active()->count('id');
            }
            if ($field == 'city_id') {
                return $query->where('city_id', '=', $value)->active()->count('id');
            }
        }
    }
