...
class User extends Authenticatable implements MustVerifyEmail
{
     ...
     */
    protected $fillable = [
        'name', 'email', 'password', 'google_id','is_active','verified','email_verified_at'
    ];
    protected $dates = ['created_at', 'updated_at', 'date_of_birth', 'package_start_date', 'package_end_date','email_verified_at'];
    ...
    {
        return $this->hasMany('App\UserJobTitle', 'user_id', 'id');
    }

    public function userJobTypes()
    {
        return $this->hasMany('App\UserJobType', 'user_id', 'id');
    }
    ...
    {
        return $this->belongsTo('App\DegreeLevel', 'degree_level_id', 'id');
    }

    public function salaryPeriod()
    {
        return $this->belongsTo('App\SalaryPeriod', 'salary_period_id', 'id');
    }
    ...
    public function getSocialMedia($field = '')
    {
        $socialMedia = $this->socialMedia()->lang()->first();
        if (null === $socialMedia) {
            $socialMedia = $this->socialMedia()->first();
        }
        if (null !== $socialMedia) {
            if (!empty($field))
                return $socialMedia->$field;
            else
                return $socialMedia;
        }
    }
