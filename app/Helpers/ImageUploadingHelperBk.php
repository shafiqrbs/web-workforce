...
class ImageUploadingHelperBk
{

    private static $mainImgWidth = 900;
    private static $mainImgHeight = 500;
...
////        dd(self::$mainImgWidth, self::$mainImgHeight);
    ...
    public static function UploadImage($destinationPath, $field, $newName = '', $width = 0, $height = 0, $makeOtherSizesImages = true)
    {
        ...
        if ($makeOtherSizesImages === true) {
            $imageToResize->resize(self::$midImgWidth, self::$midImgHeight, function ($constraint) {
                /*$constraint->aspectRatio();
                $constraint->upsize();*/
            })->resizeCanvas(350, 300)->save($midImagePath . '/' . $fileName);
            $imageToResize->resize(self::$thumbImgWidth, self::$thumbImgHeight, function ($constraint) {
                /*$constraint->aspectRatio();
                $constraint->upsize();*/
            })->resizeCanvas(150, 150)->save($thumbImagePath . '/' . $fileName);
            /*             * **** End Resizing Images ******** */
        }
        return $fileName;
    }
