...
use App\Http\Requests\OfficeAdministrationFormRequest;
...
class CommitteeOfficeAdministrationController extends Controller
{
    /**
     ...
     *
     */
    public function indexOfficeAdministrationMembers()
    {
        return view('admin.committee_member.office_administration.index');
    }


    public function fetchOfficeAdministrationMembersData(Request $request)
    {
        ...
        $committeeMembers = CommitteeMember::select(
            ...
            ->where('committee_members.committee_type','=','OFFICE_ADMINISTRATION');
            ...
            ->addColumn('action', function ($committeeMembers) {
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
							<a href="' . route('edit.office.administration.member', ['id' => $committeeMembers->id]) . '"><i class="fa fa-pencil" aria-hidden="true"></i>Edit</a>
						</li>	
    ...
    public function createOfficeAdministrationMember()
    {
        $designations = DataArrayHelper::langCareerLevelsArrayByCommitteeType('OFFICE_ADMINISTRATION');

        return view('admin.committee_member.office_administration.add')
            ->with('designations',$designations);
    }

    public function storeOfficeAdministrationMember(OfficeAdministrationFormRequest $request)
    {
        ...
        $executiveMember->committee_type = 'OFFICE_ADMINISTRATION';
        $executiveMember->save();
        /*         * ************************************ */
        $executiveMember->sort_order = $executiveMember->id;
        $executiveMember->update();
        flash('Office Administration member has been added!')->success();
        return \Redirect::route('edit.office.administration.member', array($executiveMember->id));
    }

    public function editOfficeAdministrationMember($id)
    {

        $executiveMember = CommitteeMember::findOrFail($id);
        $designations = DataArrayHelper::langCareerLevelsArrayByCommitteeType('OFFICE_ADMINISTRATION');

        return view('admin.committee_member.office_administration.edit')
            ->with('designations',$designations)
            ->with('executiveMember',$executiveMember);
    }


    public function updateOfficeAdministrationMember($id, OfficeAdministrationFormRequest $request)
    {
        ...
        $executiveMember->committee_type = 'OFFICE_ADMINISTRATION';
        $executiveMember->update();
        flash('Office Administration member has been updated!')->success();
        return \Redirect::route('edit.office.administration.member', array($executiveMember->id));
    }


    public function deleteOfficeAdministrationMember(Request $request)
    {
        $id = $request->input('id');
        try {
    ...
    public function sortOfficeAdministrationMembers()
    {
        return view('admin.committee_member.office_administration.sort');
    }

    public function officeAdministrationMemberSortData(Request $request)
    {
    ...
    public function officeAdministrationMemberSortUpdate(Request $request)
    {
    ...
    public function makeActiveOfficeAdministration(Request $request)
    {
        $id = $request->input('id');
        try {
    ...
    public function makeNotActiveOfficeAdministration(Request $request)
    {
        $id = $request->input('id');
        try {
            $archive = CommitteeMember::findOrFail($id);
