...
class LanguageController extends Controller
{
    ...
    public function sortLanguage()
    {
        $languages = DataArrayHelper::languagesNativeCodeArray();
        return view('admin.language.sort')->with('languages', $languages);
    }

    public function languageSortData(Request $request)
    {
        $languageLevels = Language::select('languages.id', 'languages.native', 'languages.lang', 'languages.sort_order')
        ...
        if ($languageLevels != null) {
            foreach ($languageLevels as $languageLevel) {
                $str .= '<li id="' . $languageLevel->id . '"><i class="fa fa-sort"></i>' . $languageLevel->native . '</li>';
            }
        }
        echo $str . '</ul>';
    }

    public function languageSortUpdate(Request $request)
    {
        $languageLevelOrder = $request->input('languageOrder');
        $languageLevelOrderArray = explode(',', $languageLevelOrder);
        $count = 1;
        foreach ($languageLevelOrderArray as $languageLevelId) {
            $languageLevel = Language::find($languageLevelId);
