...
class CmsController extends Controller
{
    ...
    {
        $parentCms= Cms::where('parent_id','=',null)->pluck('seo_title','id');
        return view('admin.cms.add',['parentCms'=>$parentCms]);
    }
    ...
    public function storeCms(CmsFormRequest $request)
    {
        ...
        $cms->parent_id = $request->input('parent_id');
    ...
    public function editCms($id)
    {
        ...
        return view('admin.cms.edit', compact(['cms','parentCms']));
    }
    ...
    public function sortCms()
    {
        return view('admin.cms.sort');
    }

    public function cmsSortData(Request $request)
    {
        $lang = $request->input('lang');
        $cms = Cms::select('cms.id', 'cms.seo_title', 'cms.sort_order')
            ->where('cms.show_in_top_menu', 1)
            ->orderBy('cms.sort_order')
            ->get();
        $str = '<ul id="sortable">';
        if ($cms != null) {
            foreach ($cms as $data) {
                $str .= '<li id="' . $data->id . '"><i class="fa fa-sort"></i>' . $data->seo_title . '</li>';
            }
        }
        echo $str . '</ul>';
    }

    public function cmsSortUpdate(Request $request)
    {
        $cmsOrder = $request->input('cmsOrder');
        $cmsOrderArray = explode(',', $cmsOrder);
        $count = 1;
        foreach ($cmsOrderArray as $cmsId) {
            $cms = Cms::find($cmsId);
            $cms->sort_order = $count;
            $cms->update();
            $count++;
        }
    }
