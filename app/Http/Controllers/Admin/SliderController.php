...
class SliderController extends Controller
{
    ...
    public function storeSlider(SliderFormRequest $request)
    {
		...
		if ($request->hasFile('slider_image')) {
			...
			$fileName = ImgUploader::UploadImage('slider_images', $request->file('slider_image'), $image_name, 1920, 832);
    ...
    public function fetchSlidersData(Request $request)
    {
        ...
        $sliders = Slider::select([
            \Illuminate\Support\Facades\DB::raw('@rownum  := @rownum  + 1 AS rownum'),
                    'sliders.id', 'sliders.lang', 'sliders.slider_heading', 'sliders.is_default', 'sliders.slider_id', 'sliders.is_active','slider_image','slider_link'
                ]);
        return Datatables::of($sliders)
                        ...
                        ->addColumn('slider_image', function($row){
                            $banner_image='';
                            if($row->slider_image){
                                $banner_image.='<img src="'.asset("slider_images/thumb/{$row->slider_image}").'" alt="" >';
                            }
                            return $banner_image;
                        })
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li>
						<a href="javascript:void(0);" onClick="' . $activeHref . '" id="onclickActive' . $sliders->id . '"><i class="fas fa-check-square"></i>' . $activeTxt . '</a>
						</li>						
						<li>
							<a href="javascript:void(0);" onclick="deleteSlider(' . $sliders->id . ', ' . $sliders->is_default . ');" class=""><i class="fa fa-trash" aria-hidden="true"></i>Delete</a>
						</li>
																																							
					</ul>
				</div>';
                        })
                        ->rawColumns(['action', 'slider_heading','slider_image'])
    ...
    {
        return view('admin.slider.sort');
    }

    public function sliderSortData(Request $request)
    {
        $sliders = Slider::select('sliders.id', 'sliders.slider_heading', 'sliders.sort_order')->where('is_active',1)
