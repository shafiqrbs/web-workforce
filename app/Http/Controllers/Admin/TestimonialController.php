...
class TestimonialController extends Controller
{
    ...
    public function fetchTestimonialsData(Request $request)
    {
        $testimonials = Testimonial::select([
                    'testimonials.id','testimonials.rating', 'testimonials.created_at', 'testimonials.user_type', 'testimonials.lang', 'testimonials.testimonial_by', 'testimonials.user_id', 'testimonials.company', 'testimonials.testimonial', 'testimonials.is_default', 'testimonials.testimonial_id', 'testimonials.is_active',
                ])->join('users', 'users.id', '=', 'testimonials.user_id')->sorted();
        return Datatables::of($testimonials)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('testimonial_by') && !empty($request->testimonial_by)) {
                                ...
                                $query->where('users.first_name', 'like', "%{$request->get('testimonial_by')}%");
                            }
                            ...
                            if ($request->has('user_type') && $request->user_type != '') {
                                $query->where('testimonials.user_type', '=', "{$request->get('user_type')}");
                            }
                        })
                        ...
                        ->addColumn('testimonial_by', function ($testimonial){
                        ...
                        ->addColumn('user_id', function ($testimonial){
                            $userId= $testimonial->user_id;
                            if($userId){
                                $user = User::find($userId);
                        ...
                        ->addColumn('user_type', function ($testimonial){
                            return ucfirst($testimonial->user_type);
                        })
                        ->addColumn('created_at', function ($testimonial){
                            return $testimonial->created_at->format('d M Y');
                        })
                        ->addColumn('is_active', function ($testimonial){
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						<li>
                                        <a href="' . route('view.testimonial', ['id' => $testimonials->id]) . '"><i class="fa fa-eye" aria-hidden="true"></i>View</a>
                                        </li>						
						<li>
