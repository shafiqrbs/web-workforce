    ...
    <div class="page-content"> 
        ...
        <div class="page-bar">
            <ul class="page-breadcrumb">
                ...
                <li> <span>Camp Commandants</span> </li>
            </ul>
        </div>
        <!-- END PAGE BAR --> 
        <!-- BEGIN PAGE TITLE-->
            ...
            <div class="col-md-12"> 
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">Camp Commandants</span> </div>
                        <div class="actions">
                            <a href="{{ route('create.camp.commandant.member') }}" class="btn btn-xs btn-success"><i class="glyphicon glyphicon-plus"></i> Add New Camp Commandant</a>
                        </div>
                    </div>
...
<script>
    $(function () {
        var oTable = $('#executive_member_ajax').DataTable({
            ...
            ajax: {
                url: '{!! route('fetch.data.camp.commandant.members') !!}',
                data: function (d) {
    ...
    function delete_committee_member(id) {
        if (confirm('Are you sure! you want to delete?')) {
            $.post("{{ route('delete.camp.commandant.member') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
    ...
    function make_not_active(id) {
        ...
        $.post("{{ route('make.not.active.camp.commandant') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
            .done(function (response) {
    ...
    function make_active(id) {
        ...
        $.post("{{ route('make.active.camp.commandant') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
            .done(function (response) {
                if (response.status == 'ok')
                {
