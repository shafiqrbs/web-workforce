            ...
            <div class="col-md-12">
                ...
                <div class="portlet light portlet-fit portlet-datatable bordered">
                    <div class="portlet-title">
                        <div class="caption"> <i class="icon-settings font-dark"></i> <span class="caption-subject font-dark sbold uppercase">Manage Executive Committee</span> </div>
                        <div class="actions">
...
<script>
    $(function () {
        var oTable = $('#executive_member_ajax').DataTable({
            ...
            ajax: {
                url: '{!! route('fetch.data.executive.committee.members') !!}',
                data: function (d) {
                    d.lang = $('#lang').val();
    ...
    function delete_committee_member(id) {
        if (confirm('Are you sure! you want to delete?')) {
            $.post("{{ route('delete.executive.committee.member') }}", {id: id, _method: 'DELETE', _token: '{{ csrf_token() }}'})
    ...
    function make_not_active(id) {
        ...
        $.post("{{ route('make.not.active.executive.committee') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
            .done(function (response) {
    ...
    function make_active(id) {
        ...
        $.post("{{ route('make.active.executive.committee') }}", {id: id, _method: 'PUT', _token: '{{ csrf_token() }}'})
            .done(function (response) {
                if (response.status == 'ok')
                {
